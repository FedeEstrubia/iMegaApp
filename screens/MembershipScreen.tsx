...
const MembershipScreen: React.FC = () => {
  ...
  const tiers = [
    { name: 'Bronce', points: '0 - 499 pts', discount: '0%', features: ['Acceso Estándar', 'Envío Estándar Gratis'], current: false },
    { name: 'Plata', points: '500 - 1,999 pts', discount: '5%', features: ['Soporte Prioritario', 'Acceso Anticipado a Ofertas'], current: true },
    { name: 'Oro', points: '2,000 - 4,999 pts', discount: '10%', features: ['Envío Express Gratis', 'Acceso a Eventos Exclusivos'], current: false }
  ];
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <div className="p-4 space-y-6 overflow-y-auto no-scrollbar">
         ...
         <div className="relative overflow-hidden rounded-2xl bg-surface-dark p-6 shadow-lg">
            <div className="relative z-10 flex flex-col gap-4">
              <div>
                ...
                <h1 className="text-2xl font-bold text-white">Nivel Plata</h1>
              </div>
              <div className="flex flex-col gap-2">
                <div className="flex justify-between text-sm">
                  <span className="text-white">1,250 Puntos</span>
                  <span className="text-slate-400">Meta: 2,000</span>
                </div>
         ...
         <div className="space-y-4">
          {tiers.map(tier => (
            <div key={tier.name} className={`relative flex flex-col gap-4 rounded-xl border p-5 ${tier.current ? 'border-primary bg-white dark:bg-surface-dark shadow-lg ring-2 ring-primary/20' : 'border-slate-200 dark:border-slate-700'}`}>
              {tier.current && (
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h4 className={`text-lg font-bold ${tier.current ? 'text-primary' : ''}`}>{tier.name}</h4>
                  <span className="text-xs text-slate-500">{tier.points}</span>
                </div>
                <div className="flex items-baseline gap-1">
                  <span className={`text-3xl font-black ${tier.current ? 'text-primary' : 'text-slate-400'}`}>{tier.discount}</span>
